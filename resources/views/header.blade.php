
<div id="contactbar">
    <div id="headaddress">
        <i class="ri-map-pin-2-line"></i>
        <p>Shop No-123, 1st-Floor, Mahagun Mart, Sector-16C, Greater Noida West, Gautam Buddha Nagar, UP - 201306</p>
    </div>
    <div id="headmail">
        <i class="ri-mail-line"></i>
        <a href="mailto:mail@yoycapital.com">mail@yoycapital.com</a>
    </div>
    <div id="social">
        <a href="https://www.justdial.com/Delhi/YOY-Capital-Infra-Pvt-Ltd-Gaur-City-2/011PXX11-XX11-190724164652-A5Z5_BZDET?trkid=&term=&ncatid=10334024&area=&search=Showing%20Results%20for%20%22Yoy%20Capital%20Infra%20Pvt%20Ltd%22%20in%20Delhi&mncatname=Yoy%20Capital%20Infra%20Pvt%20Ltd&abd_btn=&abd_heading=&isFreetxt=1&bd=2&" target="_blank"><i style="font-weight:bold;"><b>jd</b></i></i></a>
        <a href="https://www.facebook.com/yoycapitalinfra/" target="_blank"><i class="ri-facebook-box-fill"></i></a>
        <a href="https://x.com/yoycapitalinfra" target="_blank"><i class="ri-twitter-x-fill"></i></a>
        <a href="https://www.instagram.com/yoycapitalinfrapvt/" target="_blank"><i class="ri-instagram-fill"></i></a>
    </div>
    <div id="headquote">
        <a href="{{ route('contact.submit') }}" class="nav-link" data-page="contactform.php"><i class="ri-chat-quote-fill"></i>Get Quote</a>
    </div>
</div>
<div id="navbar">
    <div id="compname">
        <img id="logo" src="./images/yoylogo.png" alt="YOY Capital">
        <h1>YOY Capital <span><br>Infra Pvt. Ltd. (OPC)</span></h1>
    </div>
    <nav id="menubar">
            <li><a class="{{ request()->is('/') ? 'active' : '' }}" href="{{ route ('home') }}">Home</a></li>
            <li><a class="{{ request()->is('services') ? 'active' : '' }}" href="{{ route ('services') }}">Services</a></li>
            <li><a class="{{ request()->is('about') ? 'active' : '' }}" href="{{ route('about') }}">About Us</a></li>
            <li><a class="{{ request()->is('contact') ? 'active' : '' }}" href="{{ route('contact.submit') }}">Contact Us</a></li>
        <div class="hotline">
            <a id="hotlineicon" href="tel:9839193311"><i class="ri-phone-line"></i></a>
            <h4>Hotline Number <br> <span>9839193311</span></h4>
        </div>
    </nav>
   
    <div id="sidebarbtn">
        <span onclick="openNav()">&#9776;</span>
    </div>
    <path class="learnworlds-divider-fill js-learnworlds-divider-fill" d="M1236 155C876 365 285 420 0 420V468.5H1236V155Z"></path>

</div>